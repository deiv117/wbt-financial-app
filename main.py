...
if not st.session_state.user:
    ...
    if st.session_state.menu_actual == "⚙️ Perfil":
        ...
        with col_i1:
            st.info("Bájate la plantilla y rellénala.")
            st.download_button("📄 Plantilla .CSV", "fecha,cantidad,categoria\n2026-02-12,15.50,Alimentacion", "plantilla.csv")
        with col_i2:
            up = st.file_uploader("Sube tu CSV", type=["csv"])
            if up and st.button("🚀 Iniciar Proceso"):
                try:
                    ...
                    if rows: supabase.table("user_imputs").insert(rows).execute(); st.success(f"{len(rows)} importados")
        ...
        with tab_mov:
            ...
            if f_cs:
                s_c = st.selectbox("Categoría", ["Selecciona..."] + [c['name'] for c in f_cs])
                if st.button("Guardar") and s_c != "Selecciona...":
                    cid = next(c['id'] for c in f_cs if c['name'] == s_c)
                    supabase.table("user_imputs").insert({"user_id": st.session_state.user.id, "quantity": qty, "type": t_t, "category_id": cid, "date": str(f_m)}).execute(); st.rerun()
            st.divider()
        ...
        with tab_prev:
            ...
            if cat_g:
                ...
                with col_g2:
                    df_p_t = pd.DataFrame(cat_g)[['name', 'budget']].rename(columns={'name':'Categoría','budget':'Presupuesto'})
                    st.dataframe(df_p_t, hide_index=True, use_container_width=True)

        with tab_mes:
            st.subheader("Resumen Mensual")
            ml = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"]
            im1, im2 = st.columns(2)
            sm, sa = im1.selectbox("Mes", ml, index=datetime.now().month-1), im2.selectbox("Año", range(2024, 2030), index=datetime.now().year-2024)
            if not df_all.empty:
