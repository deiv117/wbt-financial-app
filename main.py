...
# --- FUNCIONES DIALOG (DEFINIDAS ANTES DE USARLAS) ---
...
if not st.session_state.user:
    ...
    with st.sidebar:
        ...
        if st.button("📂 Mis Categorías"): st.session_state.menu_actual = "📂 Categorías"; st.rerun()
    ...
    if st.session_state.menu_actual == "📂 Categorías":
            ...
            with col:
                ...
                for c in [cat for cat in current_cats if cat.get('type') == t]:
                    with st.container(border=True):
                        ...
                        if k2.button("✏️", key=f"cat_ed_{c['id']}"): editar_categoria_dialog(c)
                        if k3.button("🗑️", key=f"cat_del_{c['id']}"): 
        ...
        with st.form("perfil_form"):
            ...
            n_color = c1.color_picker("Color Avatar", value=p_data.get('profile_color', "#636EFA"))
            n_avatar = c2.text_input("URL Foto", value=p_data.get('avatar_url', ""))
            n_social = c2.toggle("Modo Social", value=p_data.get('social_active', False))
            if st.form_submit_button("Guardar"):
                supabase.table("profiles").upsert({"id": st.session_state.user.id, "name": n_name, "lastname": n_last, "avatar_url": n_avatar, "profile_color": n_color, "social_active": n_social}).execute()
                st.success("Actualizado"); st.rerun()

    elif st.session_state.menu_actual == "📥 Importar":
        st.title("📥 Importar")
        up = st.file_uploader("Archivo CSV", type=["csv"])
        if up and st.button("🚀 Iniciar"):
            try:
                ...
                if rows: supabase.table("user_imputs").insert(rows).execute(); st.success("Éxito"); st.rerun()
            except: st.error("Error en archivo")

    else:
        # --- PANEL PRINCIPAL ---
        st.title("📊 Cuadro de Mando")
        t1, t2, t3, t4, t5 = st.tabs(["💸 Movimientos", "🗄️ Historial", "🔮 Previsión", "📊 Mensual", "📅 Anual"])
        ...
        with t1:
            ...
            for i in (res_rec.data if res_rec.data else []):
                ...
                n_t = str(i.get('notes') or "")
                res_n = f" - *{n_t[:20]}...*" if n_t else ""
                cl1, cl2, cl3, cl4, cl5 = st.columns([2.5, 1, 0.8, 0.4, 0.4])
                cl1.markdown(f"**{i['date']}** | {cat_str}{res_n}")
                cl2.write(f"{i['quantity']:.2f}€")
                cl3.write("📉" if i['type'] == "Gasto" else "📈")
                if cl4.button("✏️", key=f"m_ed_{i['id']}"): editar_movimiento_dialog(i, current_cats)
                if cl5.button("🗑️", key=f"m_del_{i['id']}"): supabase.table("user_imputs").delete().eq("id", i['id']).execute(); st.rerun()

        with t2:
            st.subheader("Historial Completo")
            h1, h2 = st.columns(2)
            f_i, f_f = h1.date_input("Desde", datetime.now()-timedelta(days=30), key="h_i"), h2.date_input("Hasta", datetime.now(), key="h_f")
            if not df_all.empty:
        ...
        with t3:
            ...
            m2.metric("Ahorro Teórico", f"{(mi - tp):.2f}€")
        ...
        with t4:
            ...
            if not df_all.empty:
                ...
                if not df_m.empty:
                    ...
                    st.metric("Balance Mes", f"{(im - gm):.2f}€")
        ...
        with t5:
            ...
            if not df_all.empty:
                ...
                if not df_an.empty:
                    rm = df_an.pivot_table(index=df_an['date'].dt.month, columns='type', values='quantity', aggfunc='sum').fillna(0).reindex(range(1,13), fill_value=0)
                    fig = go.Figure()
                    if 'Ingreso' in rm.columns: fig.add_trace(go.Bar(x=ml, y=rm['Ingreso'], name='Ingreso', marker_color='#00CC96'))
                    if 'Gasto' in rm.columns: fig.add_trace(go.Bar(x=ml, y=rm['Gasto'], name='Gasto', marker_color='#EF553B'))
