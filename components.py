...
def crear_categoria_dialog(user_id):
    ...
    if st.button("Guardar"):
        if name:
            save_category({"user_id": user_id, "name": name, "type": c_type, "budget": budget, "emoji": emoji_final})
            st.rerun()
...
def editar_categoria_dialog(cat_data):
    ...
    if st.button("Actualizar Categoría"):
        if new_name:
            update_category(cat_data['id'], {"name": new_name, "emoji": emoji_final, "budget": new_budget})
            st.rerun()
