...
def render_dashboard(df_all, current_cats, user_id):
    # --- CSS DEFINITIVO Y SEGURO (Tu versión estable) ---
    st.markdown("""
        <style>
    ...
    with t1:
        ...
        # Mantenemos la mejora del formulario que se limpia solo (clear_on_submit=True)
        # porque esa parte sí funcionaba bien y es útil.
        with st.form("nuevo_mov_form", clear_on_submit=True):
            ...
            sel = st.selectbox("Categoría", opciones_cat, key="f_cat")
